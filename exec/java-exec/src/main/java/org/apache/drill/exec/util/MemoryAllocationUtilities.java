...
public class MemoryAllocationUtilities {
  ...
  public static void setupBufferedMemoryAllocations(PhysicalPlan plan, final QueryContext queryContext) {
    setupBufferedOpsMemoryAllocations(plan.getProperties().hasResourcePlan, plan.getSortedOperators(), queryContext);
  }
   ...
   * @param enableMemoryPlanning defines the memory planning needs to be done or not.
   ...
   */
  public static void setupBufferedOpsMemoryAllocations(boolean enableMemoryPlanning,
    ...
    if (!enableMemoryPlanning || bufferedOperators.isEmpty()) {
      return;
    }
