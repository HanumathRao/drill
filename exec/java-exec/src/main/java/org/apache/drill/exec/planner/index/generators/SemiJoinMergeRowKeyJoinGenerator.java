...
public class SemiJoinMergeRowKeyJoinGenerator extends NonCoveringIndexPlanGenerator {
  ...
  private List<RelNode> merge(List<RelNode> leftSideRels, List<RelNode> rightSideRels) {
    ...
    for (int i=0;i<largest;i++) {
      ...
      RelNode leftRel = i < leftRels.size() ? leftRels.get(i) : buildProject(lastValidLeftInput);
      RelNode rightRel = i < rightRels.size() ? rightRels.get(i) : buildProject(lastValidRightInput);
      result.addAll(merge(leftRel, rightRel, result.size() == 0 ? null : result.get(result.size()-1)));
    }
    return result;
  }

  private ProjectPrel buildProject(RelNode input) {
    return new ProjectPrel(input.getCluster(), input.getTraitSet(), input,
            IndexPlanUtils.projects(input, input.getRowType().getFieldNames()),input.getRowType());
  }
...
//    if (leftRel == null) {
//      leftRel = new ProjectPrel(input.getCluster(), input.getTraitSet(), );
//      List<RelNode> inputs = Lists.newArrayList();
//      inputs.add(input);
//      result.add(rightRel.copy(rightRel.getTraitSet(), inputs));
//    } else if (rightRel == null) {
//      rightRel = new ProjectPrel(input.getCluster(), )
//      List<RelNode> inputs = Lists.newArrayList();
//      inputs.add(input);
//      result.add(leftRel.copy(leftRel.getTraitSet(), inputs));
//    }
//    else {
...
//    }
  ...
  private RelNode merge(ProjectPrel projectL, ProjectPrel projectR, RelNode input) {
    Set<RexNode> combinedProjects = Sets.newHashSet();
    combinedProjects.addAll(projectL.getProjects());
    combinedProjects.addAll(projectR.getProjects());
    List<RexNode> listOfProjects = Lists.newArrayList();
    listOfProjects.addAll(combinedProjects);
    return new ProjectPrel(input.getCluster(), input.getTraitSet(), input, listOfProjects, input.getRowType());
  }

  private List<RelNode> mergeFilterProjectRels(ProjectPrel lRel, FilterPrel pRel, RelNode input) {
    List<RelNode> result = Lists.newArrayList();
    input = new FilterPrel(input.getCluster(), input.getTraitSet(), input, pRel.getCondition());
    result.add(input);
    input = new ProjectPrel(input.getCluster(), input.getTraitSet(), input, lRel.getProjects(), lRel.getRowType());
    result.add(input);
    return result;
  }
