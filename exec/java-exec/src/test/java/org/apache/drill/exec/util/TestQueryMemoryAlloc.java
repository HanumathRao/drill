...
public class TestQueryMemoryAlloc extends DrillTest {
  ...
  public void testDefaultOptions() throws Exception {
    ...
    try (OperatorFixture fixture = builder.build()) {
      ...
      long mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 8 * ONE_GB);
      assertEquals(2 * ONE_GB, mem);

      // Up to 40 GB, query memory dominates.

      mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 40 * ONE_GB);
      assertEquals(2 * ONE_GB, mem);

      // After 40 GB, the percent dominates

      mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 100 * ONE_GB);
  ...
  public void testCustomFloor() throws Exception {
    ...
    try (OperatorFixture fixture = builder.build()) {
      ...
      mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 60 * ONE_GB);
      assertEquals(3 * ONE_GB, mem);
  ...
  public void testCustomPercent() throws Exception {
    ...
    try (OperatorFixture fixture = builder.build()) {
      ...
      mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 20 * ONE_GB);
      assertEquals(2 * ONE_GB, mem);

      // After 20 GB, the percent dominates

      mem = DefaultRMMemoryAllocationUtilities.computeQueryMemory(fixture.config(), optionManager, 30 * ONE_GB);
      assertEquals(3 * ONE_GB, mem);
    }
  }
  ...
  public void testOpMemory() throws Exception {
    ...
    try (OperatorFixture fixture = builder.build()) {
      ...
      long opMinMem = DefaultRMMemoryAllocationUtilities.computeOperatorMemory(optionManager, 4 * ONE_GB, 2);
      assertEquals(4 * ONE_GB / 10 / 2, opMinMem);

      // Too little memory per operator. Use configured minimum.

      opMinMem = DefaultRMMemoryAllocationUtilities.computeOperatorMemory(optionManager, ONE_GB, 100);
